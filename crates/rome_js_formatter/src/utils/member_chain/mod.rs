...
use rome_formatter::{format_args, write, Buffer, CstFormatContext, PreambleBuffer};
use rome_js_syntax::{
...
fn get_call_expression_groups(
    ...
    flatten_call_expression(&mut flattened_items, syntax_node, f.context())?;
...
fn flatten_call_expression(
    ...
    match node.kind() {
        JsSyntaxKind::JS_CALL_EXPRESSION => {
            ...
            flatten_call_expression(queue, callee.syntax(), context)?;
        ...
        JsSyntaxKind::JS_STATIC_MEMBER_EXPRESSION => {
            ...
            flatten_call_expression(queue, object.syntax(), context)?;
